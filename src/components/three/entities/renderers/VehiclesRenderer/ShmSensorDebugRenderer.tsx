// ShmSensorDebugRenderer.tsx - Wireframe visualization of vehicle sensors (SHM mode)
...
import {
  ...
  SENSOR_POINT_SIZE,
  SensorPoint,
} from "@/shmSimulator/memory/sensorPointArray";
import { getMarkerConfig } from "@/config/mapConfig";

interface ShmSensorDebugRendererProps {
  readonly numVehicles: number;
}

/**
 * Render sensor wireframes for debugging (SHM mode)
 * Shows 3-zone sensor quads (outer/approach, middle/brake, inner/stop) and body quad
 */
export function ShmSensorDebugRenderer({ numVehicles }: ShmSensorDebugRendererProps) {
