...
### Q14. projected_demand 윈도우는 몇 개, 어떤 간격으로 끊나? (30/60/180 vs 15초 균등)

**결론: 로그에서 정하지 않는다. Route 로그는 raw committed path만 남기고,
윈도우는 offline feature 단계의 튜닝거리로 남긴다.**

이유 — `projected_demand_Ns`는 Route 로그를 forward-walk해서 **offline에서 계산하는
파생값**이다 (Q13). 윈도우를 15초로 끊든 30/60/180으로 하든 Python feature 스크립트만
바꾸면 된다. **로그 포맷과 무관.** 그러니 로깅 시점에 윈도우 집합을 확정할 필요 없음 —
Route 로그는 `(ts, veh_id, edge 순서)`만 남긴다.

**그래도 윈도우 집합을 고를 때의 지침:**

1. **균등 15초 × 20개(→300초)는 비추천.**
   - 누적 윈도우라 인접끼리 중첩: `proj_demand_15 ⊂ _30 ⊂ _45 ...` → 옆 컬럼과 값이
     거의 같다. 20개 중 대부분이 잉여 → 과적합 표면만 넓히고 ablation만 지저분.
   - 먼 윈도우는 저신호: forward-walk는 nominal 속도 + 미래 리라우팅/lock 무시 가정이라
     horizon이 길수록 추정이 소설이 된다. `proj_demand_300`은 garbage-in.

2. **비균등 확장 간격이 맞다.** 정보 밀도가 horizon 따라 감소하므로 가까운 미래는 촘촘,
   먼 미래는 성기게. 30/60/180이 "들쭉날쭉"해 보여도 원리는 옳다(다만 60→180 점프는
   문서가 대충 잡은 값).

3. **시작값(의도): `15 / 30 / 60 / 120` 4개**, 확장 간격. stage 1은 이걸로 출발.
   최종 컷오프·개수는 직관(300초)이 아니라 **ablation으로 신호가 죽는 지점을 찾아** 정한다.
   edge 통과가 수~수십 초, route 전체가 1~2분 규모면 그 너머 윈도우는 변별력이 사라진다.
